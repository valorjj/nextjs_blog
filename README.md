...
-   1년, 아니 달 단위로 업데이트 되는 Next.js 속도에 맞추기가 어렵다.
    -   더 이상 유효하지 글이 너무나 많다.
-   React.js 에서 복잡하게 해야하는 작업이 Next.js 에선 라이브러리로 제공되는 경우가 있다.
    -   에러가 발생했을 때, 내부 로직을 수정할 수 없다면 프로젝트 전체가 영향을 받는다.
    -   초보에게는 오히려 너무 간단하게 사용하는 건 독이라고 느껴졌다.
-   스프링부트 + MySQL + Spring Security 와는 다르게 설정이 쉽고 편하다.
    -   이미 만들어진 라이브러리가 강력하다.
-   NoSQL 은 사용이 자유롭다.
    -   제약조건, 컬렉션 등의 생성, 업데이트에 자유도가 높다.
    -   대신 실제 운영 환경에서는 수 많은 컬렉션과 도큐먼트의 관리가 어려울 것 같다.
    -   typescript 도입은 필수이다.
-   JPA 를 써보니, Prisma 라는 ORM 사용도 수월했다.
    -   선언방식, 사용법이 다른거지 하는 일은 동일해서 쉽게 익힐 수 있었다.
-   tailwind 사용하지 않고 css 로 하니 굉장히 번거롭다.
    -   대신 .jsx 파일은 깨끗하게 유지할 수 있다.
    -   tailwind 같은 css 프레임워크를 도입하지 않으려면 scss 는 필수라고 느꼈다.
